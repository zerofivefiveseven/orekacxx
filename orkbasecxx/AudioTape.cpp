...
AudioTapeDescription::AudioTapeDescription() {
    ...
    if (CaptureEvent::AudioKeepDirectionIsDefault(CONFIG.m_audioKeepDirectionIncomingDefault) == false) {
        m_audioKeepDirectionEnum = static_cast<CaptureEvent::AudioKeepDirectionEnum>(
            CaptureEvent::AudioKeepDirectionToEnum(
...
void AudioTapeDescription::Define(Serializer *s) {
    ...
    s->EnumValue("direction", reinterpret_cast<int &>(m_direction), CaptureEvent::DirectionToEnum,
                 CaptureEvent::DirectionToString);
    s->EnumValue("localside", reinterpret_cast<int &>(m_localSide), StringToEnumFunction(CaptureEvent::LocalSideToEnum),
                 CaptureEvent::LocalSideToString);
    s->EnumValue("audiokeepdirection", reinterpret_cast<int &>(m_audioKeepDirectionEnum),
                 StringToEnumFunction(CaptureEvent::AudioKeepDirectionToEnum),
                 CaptureEvent::AudioKeepDirectionToString);
    ...
    if (m_state == StateCreated || m_state == StateActive) {
        ...
        if (m_chunkQueueDataSize >= (CONFIG.m_captureFileBatchSizeKByte * 3 * 1024)) {
            if (m_chunkQueueErrorReported == false) {
                ...
                LOG4CXX_ERROR(LOG.tapeLog,
                              "[" + m_trackingId +
                              "]: Rejected additional chunk due to slow hard drive -- Queued Data Size:" +
                              FormatDataSize(m_chunkQueueDataSize) + " is greater than 3*CaptureFileBatchSizeKByte:" +
                              FormatDataSize(CONFIG.m_captureFileBatchSizeKByte * 3 * 1024));
            }
            m_RejectedCount++;
...
void AudioTape::Write() {
                ...
                if (m_state == StateActive) {
                    if ((m_bytesWritten / 1024) > CONFIG.m_captureFileSizeLimitKb) {
                        if ((time(nullptr) - m_lastLogWarning) > 3600) {
                            ...
                            logMsg.Format("[%s] capture file %s.mcf is over size limit (%u KBytes) - ignoring new data",
                                          m_trackingId, GetIdentifier(), CONFIG.m_captureFileSizeLimitKb);
                            LOG4CXX_ERROR(LOG.tapeLog, logMsg);
                            m_lastLogWarning = time(NULL);
                        }
                    } else {
                        //отправлять кусочек аудио тут
    ...
    if ((m_shouldStop && m_state != StateStopped) || m_state == StateError) {
        ...
        if (m_audioFileRef.get()) {
            //отпускать канал тут, отправлять оставшуюся метадату тоже тут
...
void AudioTape::AddCaptureEvent(CaptureEventRef eventRef, bool send) {
    ...
    switch (eventRef->m_type) {
        ...
        case CaptureEvent::EtStop:
            m_shouldStop = true;
            logMsg.Format("[%s]: pushcount:%d popcount:%d highmark:%d rejectedcount:%d", m_trackingId, m_pushCount,
                          m_popCount, m_highMark, m_RejectedCount);
            LOG4CXX_DEBUG(LOG.tapeLog, logMsg);
            m_duration = eventRef->m_timestamp - m_beginDate;
            if ((CONFIG.m_remotePartyMaxDigits > 0) && (m_remoteParty.length() > CONFIG.m_remotePartyMaxDigits) && (
                    m_remoteParty.CompareNoCase(m_remoteIp) != 0)) {
        ...
        case CaptureEvent::EtAudioKeepDirection:
            m_audioKeepDirectionEnum = static_cast<CaptureEvent::AudioKeepDirectionEnum>(
                CaptureEvent::AudioKeepDirectionToEnum(eventRef->m_value));

            audioDirectionMarks.reset(new AudioDirectionMarks());
            audioDirectionMarks->m_timestamp = time(nullptr);
            audioDirectionMarks->m_audioKeepDirectionEnum = m_audioKeepDirectionEnum;
            if (!m_audioDirectionMarks.empty())
            //This is not the first kept-direction report, so let it be the end-mark for the previous interval
            {
        ...
        case CaptureEvent::EtRemoteParty: {
            ...
            for (auto it = CONFIG.m_partyFilterMap.begin(); it != CONFIG.m_partyFilterMap.end(); ++it) {
                int pos;
        ...
        if (eventRef->m_type == CaptureEvent::EtKeyValue && !eventRef->m_value.empty() && !eventRef->m_key.empty()) {
            ...
            auto i = m_tags.find(eventRef->m_key);

            if (i == m_tags.end()) {
                m_tags.insert(std::make_pair(eventRef->m_key, eventRef->m_value));
            } else {
                std::list<CStdString>::iterator it;
                for (it = CONFIG.m_tagsListUseInitialValue.begin(); it != CONFIG.m_tagsListUseInitialValue.end(); it++)
                // Check if m_tagsListUseInitialValue contains the key to be updated
                {
                    if (*it == eventRef->m_key)
                        break;
                }

                if (it == CONFIG.m_tagsListUseInitialValue.end())
                // Iterator hit the end which means the key is not in the list, we can update the value
                {
...
void AudioTape::GetMessage(MessageRef &msgRef) {
    ...
    if (captureEventRef.get() == nullptr) {
    ...
    } else if (captureEventRef->m_type == CaptureEvent::EtStop || captureEventRef->m_type == CaptureEvent::EtStart ||
               captureEventRef->m_type == CaptureEvent::EtUpdate || captureEventRef->m_type == CaptureEvent::EtHold ||
               captureEventRef->m_type == CaptureEvent::EtResume) {
...
void AudioTape::GenerateCaptureFilePathAndIdentifier() {
    ...
    m_fileIdentifier.Format("%.4d%.2d%.2d_%.2d%.2d%.2d_%s", year, month, date.tm_mday, date.tm_hour, date.tm_min,
                            date.tm_sec, m_portId);
...
void AudioTape::GenerateFinalFilePath() {
    if (CONFIG.m_tapePathNaming.size() > 0) {
        ...
        if (pathIdentifier.size() > 0) {
            ...
            FileRecursiveMkdir(mkdirPath, CONFIG.m_audioFilePermissions, CONFIG.m_audioFileOwner,
                               CONFIG.m_audioFileGroup, CONFIG.m_audioOutputPath);
        }
    }
}
...
void AudioTape::SetExtension(const CStdString &ext) {
    m_fileExtension = ext;
}

bool AudioTape::IsStoppedAndValid() const {
...
void AudioTape::PopulateTag(const CStdString &key, const CStdString &value) {
