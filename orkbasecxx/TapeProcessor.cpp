...
void TapeProcessor::SetNextProcessor(const TapeProcessorRef &nextProcessor) {
    m_nextProcessor = nextProcessor;
}

void TapeProcessor::RunNextProcessor(AudioTapeRef &tape) {
    if (m_nextProcessor.get()) {
        if (tape->m_isExternal && tape->m_mediaType != MediaType::AudioType && tape->m_mediaType != MediaType::VideoType
            && m_nextProcessor->GetName().CompareNoCase("CommandProcessing") == 0)
        //Skip CommandProcessing (where transcoding happens in case of external audio) if the tape is Instant message
        {
...
TapeProcessorRef TapeProcessorRegistry::GetNewTapeProcessor(CStdString &TapeProcessorName) {
    for (const auto &TapeProcessor: m_TapeProcessors) {
        if (TapeProcessor->GetName().CompareNoCase(TapeProcessorName) == 0) {
            return TapeProcessor->Instanciate();
        }
    }
    return {}; // No TapeProcessor found
}

TapeProcessorRegistry *TapeProcessorRegistry::m_singleton = nullptr;
...
void TapeProcessorRegistry::CreateProcessingChain() {
    ...
    for (auto it = CONFIG.m_tapeProcessors.begin(); it != CONFIG.m_tapeProcessors.end(); it++) {
        CStdString tapeProcessorName = *it;
        TapeProcessorRef processor = GetNewTapeProcessor(tapeProcessorName);
        if (processor.get()) {
            if (m_firstTapeProcessor.get() == nullptr) {
            ...
            LOG4CXX_ERROR(
                s_log,
                CStdString("Processor:") + tapeProcessorName +
                " does not exist, please check <TapeProcessors> in config.xml");
        }
    }
}
...
void TapeProcessorRegistry::RunProcessingChain(AudioTapeRef &tape) {
    if (m_firstTapeProcessor.get()) {
        ...
        {
            if (m_firstTapeProcessor->GetName().CompareNoCase("BatchProcessing") == 0)
            //Skip BatchProcessing tape processor for imported tape
            {
