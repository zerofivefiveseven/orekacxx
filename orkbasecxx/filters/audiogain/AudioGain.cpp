...
void AudioGainFilter::AudioChunkIn(AudioChunkRef &inputAudioChunk) {
    ...
    if (inputAudioChunk->GetEncoding() != PcmAudio) {
        if (m_numEncodingErrors == 0) {
            ...
            logMsg.Format("Unexpected encoding:%d expected:%d (PcmAudio), gain not applied",
                          inputAudioChunk->GetEncoding(), PcmAudio);
            LOG4CXX_WARN(m_log, logMsg);
        }
        m_numEncodingErrors++;
    ...
    multiplier = std::pow(10, (CONFIG.m_audioGainDb / 20.0));
    multiplier1 = std::pow(10, (CONFIG.m_audioGainChannel1Db / 20.0));
    multiplier2 = std::pow(10, (CONFIG.m_audioGainChannel2Db / 20.0));
